...
def test_llm_plan_executor_sends_email(monkeypatch: MonkeyPatch) -> None:
    ...
    llm = QueueLLM([
        ...
        '{"action": "email__send_email", "action_input": '
        '{"recipients": ["alex.smith@example.com"], "subject": "Lunch follow-up", '
        '"content": "Shall we confirm the time?"}}<end_action>',
        "Thought: The task is complete.\nAction:\n"
        '{"action": "final_answer", "action_input": '
        '{"answer": "Email sent to alex.smith@example.com"}}<end_action>',
    ])
