...
def _format_timestamp(ts: float | int | None) -> str | None:
    if ts is None:
        return None
...
class AgendaView(AppState):
    ...
    @user_tool()
    def list_events(self, offset: int = 0, limit: int = 10) -> dict:
        ...
        events: list[CalendarEvent] = result.get("events", [])  # type: ignore[arg-type]
        filtered = events
        if self.tag_filter:
        ...
        return {
            "events": filtered,
            "range": result.get("range"),  # type: ignore[arg-type]
            "total": len(filtered),
        }
    ...
    def search_events(self, query: str) -> list[CalendarEvent]:
        ...
        for event in matches:
            ...
            if self.tag_filter and event.tag != self.tag_filter:
                continue
            if self.attendee_filter and not any(
                att.lower() == self.attendee_filter.lower() for att in event.attendees
            ):
                continue
    ...
    def filter_by_attendee(self, attendee: str) -> list[CalendarEvent]:
        ...
        return [
            event
            for event in self._events_in_window()
            if any(att.lower() == attendee_lower for att in event.attendees)
        ]
    ...
    @user_tool()
    def read_today_calendar_events(self) -> dict:
    ...
    @user_tool()
    def set_day(self, date: str) -> dict:
...
class EventDetail(AppState):
    ...
    def delete_by_attendee(self, who_delete: str) -> str:
        ...
        return self.app.delete_calendar_event_by_attendee(
