...
class PASNotificationSystem(VerboseNotificationSystem):
    ...
    def __init__(self, verbosity_level: VerbosityLevel = VerbosityLevel.MEDIUM) -> None:
        """Initialize the PAS notification system.
    ...
    def _split_message(self, message: Message) -> tuple[Message, Message]:
        ...
        user_message = replace(message, message_type=PASMessageType.ENVIRONMENT_NOTIFICATION_USER)
        agent_message = replace(message, message_type=PASMessageType.ENVIRONMENT_NOTIFICATION_AGENT)
    ...
    def convert_to_message(self, event: CompletedEvent) -> tuple[Message | None, Message | None] | None:
        ...
        if (
            ...
            return (
                None,
                Message(
                    message_type=PASMessageType.USER_ACTION,
                    message=message,
                    timestamp=datetime.fromtimestamp(timestamp, tz=UTC),
                ),
            )
        ...
        if should_notify:
            ...
            return (
                Message(
                    message_type=PASMessageType.ENVIRONMENT_NOTIFICATION_USER,
                    message=user_message,
                    timestamp=datetime.fromtimestamp(timestamp, tz=UTC),
                ),
                Message(
                    message_type=PASMessageType.ENVIRONMENT_NOTIFICATION_AGENT,
