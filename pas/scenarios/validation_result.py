"""PAS-specific validation result classes for scenario execution."""
...
# Schema for PAS result DataFrames - used by to_polars() and combine_results_to_dataframe()
...
class PASMultiScenarioValidationResult:
    """PAS-specific multi-scenario validation result with proactive agent metrics."""
    ...
    def add_result(self, result: PASScenarioValidationResult, scenario_id: str, run_number: int | None = None) -> None:
        """Add a scenario validation result to the multi-scenario results.

        Args:
            result: The PASScenarioValidationResult to add.
    ...
    def to_polars(self, extra_columns: dict[str, str] | None = None) -> pl.DataFrame:
        ...
        for scenario_key, scenario_result in self.scenario_results.items():
            ...
            row = {
                ...
                # Model configuration (PAS has 3 agents) - use aliases for human-readable names
    ...
    def description(
        ...
        """Generate human-readable summary with PAS metrics.
        ...
        from pas.benchmark.report_stats import generate_validation_report
