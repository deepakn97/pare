...
class ShoppingDiscountCalendarReminder(PASScenario):
    ...
    def init_and_populate_apps(self, *args: Any, **kwargs: Any) -> None:
        ...
        hike_event = CalendarEvent(
            title="Team Building Hike",
            start_datetime=datetime(2025, 12, 25, 9, 0, 0, tzinfo=UTC).timestamp(),
            end_datetime=datetime(2025, 12, 25, 14, 0, 0, tzinfo=UTC).timestamp(),
        ...
        self.calendar.events[hike_event.event_id] = hike_event

        # Populate shopping app with baseline data
        # Product 1: Hiking Boots
        boots_product = Product(name="Hiking Boots")
        boots_item = Item(
            price=120.0,
            available=True,
            options={"size": "10", "color": "brown"},
        )
        boots_product.variants[boots_item.item_id] = boots_item
        self.shopping.products[boots_product.product_id] = boots_product

        # Product 2: Water Bottle
        bottle_product = Product(name="Water Bottle")
        bottle_item = Item(
            price=25.0,
            available=True,
            options={"size": "1L", "color": "blue"},
        )
        bottle_product.variants[bottle_item.item_id] = bottle_item
        self.shopping.products[bottle_product.product_id] = bottle_product

        # Add items to cart (previously browsed but not purchased)
        self.shopping.cart[boots_item.item_id] = CartItem(
            item_id=boots_item.item_id,
            quantity=1,
            price=boots_item.price,
            available=boots_item.available,
            options=boots_item.options,
        )
        self.shopping.cart[bottle_item.item_id] = CartItem(
            item_id=bottle_item.item_id,
            quantity=2,
            price=bottle_item.price,
            available=bottle_item.available,
            options=bottle_item.options,
        )

        # Add discount code "OUTDOOR30" (30% off) applicable to both items
        # Discount expires December 22, 2025
        self.shopping.discount_codes[boots_item.item_id] = {"OUTDOOR30": 30.0}
        self.shopping.discount_codes[bottle_item.item_id] = {"OUTDOOR30": 30.0}
        ...
        self.email.folders[EmailFolderName.INBOX].add_email(welcome_email)
