...
class GroupRideCoordinationSplitPayment(PASScenario):
    ...
    7. Send a message back to the group conversation with ride confirmation details and per-person payment amount
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            e6 = (
                ...
                .oracle()
                .depends_on([e5], delay_seconds=3)
            )

            # Agent action 3: Agent sends proposal with ride option and per-person cost breakdown
            # Evidence: list_rides revealed pricing, and conversation shows 3 participants
            e7 = (
                aui.send_message_to_user(
                    content="I noticed you're planning to share a ride downtown to the Concert Venue at 6:30 PM with Alice and Bob. I found a Default ride for $15.00 total. That's $5.00 per person for the three of you. Would you like me to book this ride?",
                )
                .oracle()
                .depends_on([e6], delay_seconds=5)
            )
            ...
            e8 = (
                ...
                .oracle()
                .depends_on([e7], delay_seconds=10)
            )
            ...
            e9 = (
                ...
                .oracle()
                .depends_on([e8], delay_seconds=3)
            )

            # Agent action 5: Agent sends confirmation message to the group conversation
            # Evidence: ride was successfully booked, need to inform all participants
            e10 = (
                messaging_app.send_message_to_group_conversation(
                    conversation_id=concert_conversation_id,
                    content="Ride booked! Default ride from Downtown to Concert Venue at 6:30 PM. Total: $15.00, split $5.00 per person. See you all there!",
                )
                .oracle()
                .depends_on([e9], delay_seconds=5)
            )
    ...
    def validate(self, env: AbstractEnvironment) -> ScenarioValidationResult:  # noqa: C901
        ...
        try:
            ...
            for e in agent_events:
                if e.action.class_name == "StatefulCabApp" and e.action.function_name == "list_rides":
                    args = e.action.args
            ...
            # Check 5 (STRICT on structure, FLEXIBLE on content): Agent sent confirmation to group
            group_confirmation_found = False
            for e in agent_events:
                if (
                    ...
                    args = e.action.args
                    # Verify conversation_id is present and content exists
                    if "conversation_id" in args and args["conversation_id"] and "content" in args:
                        group_confirmation_found = True
                        break
            ...
            if not success:
                ...
                if not group_confirmation_found:
                    missing.append("agent did not send confirmation to group conversation")
