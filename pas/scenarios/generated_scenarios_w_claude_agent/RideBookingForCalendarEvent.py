...
class RideBookingForCalendarEvent(PASScenario):
    ...
    8. Monitor ride status and confirm booking

    This scenario exercises calendar event monitoring, location-based reasoning, multi-step cab booking workflow (quotation → order → status check), time-based scheduling coordination, and cross-app inference from calendar data to transportation needs..
    """
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            # Environment event: Ride status update notification (ride is confirmed)
            env_ride_status = (
                cab_app.update_ride_status(
                    status="ARRIVED_AT_PICKUP", message="Your driver has arrived at the pickup location."
                )
                .delayed(8)
                .depends_on(agent_order_ride, delay_seconds=0)
            )

            # Agent checks the current ride status to monitor the booking
            agent_check_status = cab_app.get_current_ride_status().oracle().depends_on(env_ride_status, delay_seconds=2)

            # Agent confirms the booking to the user
            agent_confirmation = (
                aui.send_message_to_user(
                    content="Your ride has been booked successfully! The driver has arrived at the pickup location. Your ride to 450 Market Street is ready."
                )
                .oracle()
                .depends_on(agent_check_status, delay_seconds=2)
            )
