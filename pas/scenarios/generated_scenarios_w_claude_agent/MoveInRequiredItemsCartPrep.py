...
class MoveInRequiredItemsCartPrep(PASScenario):
    ...
    def build_events_flow(self) -> None:
        ...
        # Extract item IDs from the populated shopping app for use in oracle events
        # This represents item IDs that the agent would discover through search/list operations
        item_ids = []
        for product in shopping_app.products.values():
            for item_id in product.variants:
                item_ids.append(item_id)

        # Assign to variables for clarity (assuming order matches seeding order)
        doormat_item_id = item_ids[0] if len(item_ids) > 0 else None
        batteries_item_id = item_ids[1] if len(item_ids) > 1 else None
        blinds_item_id = item_ids[2] if len(item_ids) > 2 else None
...
Riverside Towers Leasing Office""",
    ...
    def validate(self, env: AbstractEnvironment) -> ScenarioValidationResult:  # noqa: C901
        ...
        try:
            ...
            # Extract the item IDs that were seeded
            shopping_app = env.get_app("Shopping")
