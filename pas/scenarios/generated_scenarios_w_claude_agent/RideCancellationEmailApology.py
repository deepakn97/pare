...
class RideCancellationEmailApology(PASScenario):
    ...
    3. Immediately search for and book an alternative ride with the next available service type
    4. Calculate the new estimated arrival time based on the new ride's departure and duration
    5. Search the user's sent emails to locate the lunch confirmation thread with Rachel Thompson
    6. Reply to Rachel's email proactively, apologizing for the potential delay and providing the revised arrival time

    This scenario exercises real-time service disruption handling, cross-app coordination (cab → email), temporal reasoning to compute updated ETAs, email thread retrieval by recipient/subject search, and proactive stakeholder communication without user prompting..
    """
    ...
    def init_and_populate_apps(self, *args: Any, **kwargs: Any) -> None:
        ...
        booked_ride = Ride(
            ...
            time_stamp=datetime(2025, 11, 18, 14, 0, 0, tzinfo=UTC).timestamp(),
            distance_km=12.5,
            delay=300.0,  # 5 minutes delay in seconds
        )
        self.cab.ride_history.append(booked_ride)
        self.cab.on_going_ride = booked_ride
        # Also add to quotation history so price calculations have context
        self.cab.quotation_history.append(booked_ride)
        ...
        with EventRegisterer.capture_mode():
            ...
            reply_event = (
                email_app.reply_to_email(
                    ...
                    folder_name="SENT",
                    content="Hi Rachel,\n\nI wanted to give you a quick heads up - my original ride was cancelled, but I've booked a replacement. I may arrive a few minutes later than planned, but I'm still on track to make our 2:45 PM lunch. See you soon!\n\nBest,\nAlex",
                )
                .oracle()
    ...
    def validate(self, env: AbstractEnvironment) -> ScenarioValidationResult:
        ...
        try:
            ...
            # Accepts: reply_to_email with the correct email_id
            # This check is FLEXIBLE on the content of the reply - we only verify the action occurred
            reply_found = any(
                e.action.class_name == "StatefulEmailApp" and e.action.function_name in ["reply_to_email", "send_email"]
                for e in agent_events
            )
