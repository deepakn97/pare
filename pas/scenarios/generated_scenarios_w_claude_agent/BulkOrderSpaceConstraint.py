...
class BulkOrderSpaceConstraint(PASScenario):
    ...
    The user has multiple apartments saved to favorites with varying square footages (mix of studios, 1BR, and 2BR units). The user receives a shopping notification confirming a "bulk order" of large furniture items (couch, king bed, dining table, bookshelf) is ready for pickup or delivery. The agent must: 1) detect the bulk order confirmation notification and recognize the total volume/size implications of multiple large furniture pieces, 2) retrieve the user's saved apartments to check their square footage specifications, 3) identify which saved apartments have insufficient space (e.g., studios or small 1BR units under 500 sq ft) to reasonably accommodate all the furniture items, 4) propose removing undersized apartments from the saved list or highlighting only apartments with adequate space (e.g., 2BR units over 800 sq ft), and 5) alert the user to finalize their apartment choice from spatially compatible options before the furniture arrives.
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            env_event = shopping_app.update_order_status(order_id=self.bulk_order_id, status="shipped").delayed(
                60
            )  # Notification arrives 1 minute after scenario starts
            ...
            # Motivated by: recognizing large furniture dimensions from order details
            agent_list_saved_apts = (
                apartment_app.list_saved_apartments().oracle().depends_on(agent_get_order_details, delay_seconds=10)
            )
