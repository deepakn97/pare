...
class GroupConversationSplitPrivate(PASScenario):
    ...
    def init_and_populate_apps(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.group_conversation = ConversationV2(
            participant_ids=[self.user_id, alex_id, jordan_id, casey_id],
            title="Project Team",
    ...
    def build_events_flow(self) -> None:
        ...
        # Get Jordan's user ID for later use
        ...
        with EventRegisterer.capture_mode():
            ...
            # Agent recognizes salary/compensation discussion should not happen in group chat
            proposal_event = (
                ...
                .oracle()
                .depends_on(message2_event, delay_seconds=3)
            )
        ...
        try:
            ...
            for event in agent_events:
                if event.action.class_name == "StatefulMessagingApp" and event.action.function_name == "lookup_user_id":
                    args = event.get_args()
                    if args.get("user_name") == "Jordan Lee":
                        lookup_found = True
                        break
            ...
            # Check Step 4: Agent sent redirect message to group chat (OPTIONAL - but nice to have)
            # FLEXIBLE: This is an optional behavior not strictly required by the narrative
            # We check for it but don't make it a strict requirement for success
            group_redirect_found = False
            for event in agent_events:
                if (
                    event.action.class_name == "StatefulMessagingApp"
                    and event.action.function_name == "send_message_to_group_conversation"
                ):
                    args = event.get_args()
                    if args.get("conversation_id") == self.group_conversation.conversation_id:
                        group_redirect_found = True
                        break
