...
class BulkDiscountValidationReminder(PASScenario):
    ...
    def init_and_populate_apps(self, *args: Any, **kwargs: Any) -> None:
        ...
        printer_paper_product = Product(
            name="Premium Printer Paper",
            product_id="prod_printer_paper",
        )
        printer_paper_item = Item(
            price=24.99,
            available=True,
            item_id="item_printer_paper",
            options={"ream_count": "5", "paper_size": "Letter"},
        )
        printer_paper_product.variants["item_printer_paper"] = printer_paper_item
        self.shopping.products["prod_printer_paper"] = printer_paper_product

        # Add stapler product
        stapler_product = Product(
            name="Heavy Duty Stapler",
            product_id="prod_stapler",
        )
        stapler_item = Item(
            price=18.50,
            available=True,
            item_id="item_stapler",
            options={"color": "black", "capacity": "50 sheets"},
        )
        stapler_product.variants["item_stapler"] = stapler_item
        self.shopping.products["prod_stapler"] = stapler_product

        # Add pens product
        pens_product = Product(
            name="Ballpoint Pens Pack",
            product_id="prod_pens",
        )
        pens_item = Item(
            price=12.99,
            available=True,
        ...
        pens_product.variants["item_pens"] = pens_item
        self.shopping.products["prod_pens"] = pens_product

        # Add notebooks product
        notebooks_product = Product(
            name="Spiral Notebooks",
            product_id="prod_notebooks",
        )
        notebooks_item = Item(
            price=15.99,
            available=True,
            item_id="item_notebooks",
            options={"page_count": "100", "pack_size": "6"},
        )
        notebooks_product.variants["item_notebooks"] = notebooks_item
        self.shopping.products["prod_notebooks"] = notebooks_product

        # Add OFFICE20 discount code for all office supply items (20% off)
        self.shopping.discount_codes["item_printer_paper"] = {"OFFICE20": 20.0}
        self.shopping.discount_codes["item_stapler"] = {"OFFICE20": 20.0}
        self.shopping.discount_codes["item_pens"] = {"OFFICE20": 20.0}
        self.shopping.discount_codes["item_notebooks"] = {"OFFICE20": 20.0}
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            # Motivation: Order completed; agent updates reminder to reflect completion via discount code
            # Agent retrieves all reminders to find the office supplies reminder ID
            get_reminder_id_event = (
                reminder_app.get_all_reminders().oracle().depends_on(checkout_event, delay_seconds=1)
            )
