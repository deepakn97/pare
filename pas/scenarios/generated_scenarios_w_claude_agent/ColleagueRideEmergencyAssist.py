...
class ColleagueRideEmergencyAssist(PASScenario):
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            emergency_email = email_app.send_email_to_user_with_id(
                ...
                content="Hi! My car just broke down on the highway near Oak Valley Rest Stop. I desperately need a ride to the office at 450 Market Street for my 3:00 PM client presentation. Can you help me book a cab? I'm stranded here and my phone battery is dying. Please hurry!",
            )
            ...
            # ENVIRONMENT EVENT 2: Driver arrives at pickup location
            ride_arrived = cab_app.update_ride_status(
                status="ARRIVED_AT_PICKUP",
                message="I'm at Oak Valley Rest Stop. I see your colleague waiting.",
            ).delayed(15)

            # ORACLE EVENT 8: Agent checks updated ride status (motivated by ride status notification)
            check_arrival = cab_app.get_current_ride_status().oracle().depends_on(ride_arrived, delay_seconds=2)

            # ORACLE EVENT 9: Agent sends follow-up email to Lisa (motivated by driver arrival notification)
            followup_email = (
                email_app.send_email(
                    recipients=["lisa.park@company.com"],
                    subject="Re: URGENT: Car broke down - need ride to office!",
                    content="Hi Lisa! Quick update: Your cab has arrived at Oak Valley Rest Stop. The driver is waiting for you. Have a great presentation!",
                )
                .oracle()
                .depends_on(check_arrival, delay_seconds=3)
            )
    ...
    def validate(self, env: AbstractEnvironment) -> ScenarioValidationResult:
        ...
        try:
            ...
            # Check 2: STRICT - Agent read the emergency email to understand the request
            ...
            # Check 3: STRICT - Agent booked the ride for the correct locations
            booking_found = any(
                isinstance(e.action, Action)
            ...
            # Check 4: STRICT - Agent replied to Lisa's email with ride details (equivalence class)
            ...
            success = proposal_found and read_email_found and booking_found and reply_found
            ...
            if not success:
                ...
                if not read_email_found:
                    missing_checks.append("agent did not read emergency email")
