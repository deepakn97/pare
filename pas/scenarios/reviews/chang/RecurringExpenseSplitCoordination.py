...
class RecurringExpenseSplitCoordination(PASScenario):
    ...
    def init_and_populate_apps(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.messaging.id_to_name["user_001"] = "Me"
        self.messaging.name_to_id["Me"] = "user_001"
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            # Agent sends confirmation message to user (motivated by having created the group and reminder per user's request)
            oracle6 = (
                aui.send_message_to_user(
                    content="Setup complete! I've created a recurring monthly reminder for the 1st of each month to collect $3.75 from each person (Jordan, Casey, Alex, and you) for the $15 StreamFlix subscription. I also created a group chat titled 'Streaming Subscription Split' with all participants for coordination."
                )
                .oracle()
                .depends_on([oracle4, oracle5], delay_seconds=5)
            )

        # TODO: Register ALL events here in self.events
        self.events = [env1, env2, env3, oracle1, oracle2, oracle3, proposal, acceptance, oracle4, oracle5, oracle6]
    ...
    def validate(self, env: AbstractEnvironment) -> ScenarioValidationResult:
        ...
        try:
            ...
            # Check 1: Agent sent proposal to user mentioning all three participants and the split arrangement (STRICT on structure, FLEXIBLE on exact wording)
            proposal_found = any(
            ...
            # Check 2: Agent read at least two of the three conversations to understand commitments (FLEXIBLE: agent may use different read strategies)
            conversations_read = sum(
                1
                for e in log_entries
                ...
                in ["read_conversation", "get_conversation_by_id", "list_conversations_by_participant"]
            )
            conversations_read_found = conversations_read >= 2

            # Check 3: Agent created recurring reminder with monthly repetition (STRICT on key data: title, monthly repetition, due date)
            ...
            # Check 4: Agent created group conversation with all three participants and appropriate title (STRICT on participants)
            group_created = any(
                e.event_type == EventType.AGENT
                and isinstance(e.action, Action)
                and e.action.class_name == "StatefulMessagingApp"
            ...
            success = proposal_found and conversations_read_found and reminder_created and group_created

            if not success:
                # Build detailed rationale for failure
                missing = []
                if not proposal_found:
                    missing.append("proposal to user mentioning Jordan, Casey, Alex, and $15 split")
                if not conversations_read_found:
                    missing.append("reading at least 2 conversations to understand commitments")
                if not reminder_created:
                    missing.append("recurring monthly reminder with due date of 09:00:00 on 2025-12-01")
                if not group_created:
                    missing.append(
                        "group conversation with Jordan, Casey, and Alex titled 'Streaming Subscription Split'"
                    )
