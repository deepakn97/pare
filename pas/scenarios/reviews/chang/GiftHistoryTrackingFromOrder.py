...
class GiftHistoryTrackingFromOrder(PASScenario):
    ...
    **Actually, there's a fatal flaw:** The description says "delivery includes a gift note field mentioning 'For Sarah's birthday'" - but delivery notifications (environment events) typically just announce "Your order has arrived." The agent would need to call `get_order_details(order_id)` to see gift notes. But that requires knowing the `order_id`, which is a magic ID problem.
    ...
    def init_and_populate_apps(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.shopping.orders[electronics_order_id] = Order(
            order_id=electronics_order_id,
            order_status="shipped",
            order_date=electronics_order_date,
            order_total=129.99,
            order_items={
                electronics_item_id: CartItem(
        ...
        self.shopping.orders[groceries_order_id] = Order(
            order_id=groceries_order_id,
            order_status="shipped",
            order_date=groceries_order_date,
            order_total=45.50,
            order_items={
                groceries_item_id: CartItem(
        ...
        self.shopping.orders[apparel_order_id] = Order(
            order_id=apparel_order_id,
            order_status="shipped",
            order_date=apparel_order_date,
            order_total=89.99,
            order_items={
                apparel_item_id: CartItem(
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            # For oracle events, we retrieve the actual note_id from the seeded data
            expense_note_id = next(iter(self.note.folders["Personal"].notes.keys()))
            get_expense_note = (
                note_app.get_note_by_id(note_id=expense_note_id)
                .oracle()
    ...
    def validate(self, env: AbstractEnvironment) -> ScenarioValidationResult:
        ...
        try:
            ...
            note_updated = any(
                ...
                and e.action.args.get("note_id") == expense_note_id
