...
class ForgottenItemFriendRequest(PASScenario):
    ...
    def init_and_populate_apps(self, *args: Any, **kwargs: Any) -> None:
        ...
        # Seed a completed ride in cab history (ride that just finished at 9:00 AM)
        # This ride will be referenced when the lost item notification arrives
        completed_ride = Ride(
            status="COMPLETED",
            ...
            price=15.50,
            duration=20.0,
            time_stamp=self.start_time - 300,  # Completed 5 minutes before start_time
            distance_km=8.5,
            delay=0.0,
        )
        self.cab.ride_history.append(completed_ride)
        self.cab.quotation_history.append(completed_ride)
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            # Environment Event 1: Cab ride status update notification - item left behind
            # This is the first exogenous trigger (non-oracle environment event)
            ride_notification_event = cab_app.update_ride_status(
                status="COMPLETED",
                message="Ride completed. Did you leave anything behind? Your driver found a camera bag in the back seat. Call the cab company at +1-555-0123 to arrange the camera retrieval.",
            ).delayed(5)
            ...
            # Motivated by: ride notification mentioned lost camera bag; agent needs ride details
            ride_history_check = (
                cab_app.get_ride_history(offset=0, limit=5).oracle().depends_on(alex_message_event, delay_seconds=3)
            )
            ...
            proposal_event = (
                aui.send_message_to_user(
                    content="I noticed you left your camera bag in your recent cab ride to Downtown Coffee Shop. Alex just messaged asking to borrow your camera for a concert tonight at 5 PM. The camera is currently with your cab driver. Would you like me to help coordinate retrieving it from the cab company by calling +1-555-0123 and letting Alex know about the delay?"
                )
                .oracle()
