...
class ApartmentComparisonDecisionAid(PASScenario):
    """Agent synthesizes apartment comparison data to support user's final rental decision. The user has saved three apartments to their favorites (Oakwood Terrace, Riverside Lofts, and Sunset Meadows) and created individual tour notes in the "Personal" folder documenting each visit with details about pricing, amenities, neighborhood feel, and pet policy. After touring all three, the user receives a message notification from their property manager stating they must vacate their current unit by the end of next month, creating urgency for a decision. The agent must: 1. Detect the move-out deadline notification, 2. Search the user's saved apartments to retrieve all favorited options, 3. Search notes to locate all tour documentation by apartment name, 4. Extract and compare key decision factors (price, pet-friendliness for their dog, commute-relevant location data, lease terms) across all three apartments, 5. Create a consolidated comparison note in the "Personal" folder presenting side-by-side analysis with a recommendation based on the user's documented priorities.
        ...
        with EventRegisterer.capture_mode():
            ...
            proposal = (
                aui.send_message_to_user(
                    content="I saw your property manager's message about needing to vacate by December 31st. I noticed you have 3 apartments saved with detailed tour notes. Would you like me to create a comparison analysis to help you decide? I can synthesize your notes on pricing, pet policies for Bella, and commute factors."
                )
                .oracle()
