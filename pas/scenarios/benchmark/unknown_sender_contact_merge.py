...
class UnknownSenderContactMerge(PASScenario):
    ...
    The user receives a message from an unknown number "+1-555-0198" saying "Hey, this is Jessica Chen—I got a new phone. Please update my contact to use this number (my old one won't work). Can you send me the address for next week's event? I think you have it in my contact notes." The user has an existing contact "Jessica Chen" in their contacts app with an old phone number "+1-555-0142" and email "jessica.chen@email.com". The agent must:
    ...
    5. Send a reply message to the new number confirming the contact update and providing the requested event address information (retrieved from Jessica's contact description field which contains "Event: Community Meetup, 789 Oak Avenue, next Saturday 3 PM")
        ...
        with EventRegisterer.capture_mode():
            ...
            env_message = messaging_app.create_and_add_message(
                ...
                content=(
                    ...
                    "(my old one won't work). Can you send me the address for next week's event? "
                    "I think you have it in my contact notes."
                ),
            ).delayed(20)
            ...
            # Evidence: Original message requested the event address and explicitly suggested it's in her contact notes;
            # agent retrieved contact with description field containing the event details.
            reply_event = (
