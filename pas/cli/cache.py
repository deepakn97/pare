"""Cache CLI command for managing PAS scenario result cache."""
...
app = typer.Typer(
    name="cache",
    help="Manage PAS scenario result cache",
)
...
def _get_cache_dir() -> Path:
    ...
    env_cache_dir = os.environ.get("PAS_CACHE_DIR")
    if env_cache_dir:
...
def status() -> None:
    ...
    if os.environ.get("PAS_CACHE_DIR"):
        typer.echo("  (set via PAS_CACHE_DIR environment variable)")
...
def set_cache_dir(
    ...
    The PAS_CACHE_DIR environment variable takes precedence over this setting.
    """
    ...
    if os.environ.get("PAS_CACHE_DIR"):
        typer.echo(
            "\nNote: PAS_CACHE_DIR environment variable is set and will take precedence over this config file setting."
        )
...
def reset() -> None:
    ...
    default_cache_dir = Path.home() / ".cache" / "pas" / "scenario_results"
