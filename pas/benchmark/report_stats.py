"""Statistics and reporting utilities for PAS benchmark results."""
...
def _calculate_pas_totals(df: pl.DataFrame) -> dict[str, int]:
    """Calculate PAS total counts.
...
def calculate_statistics(df: pl.DataFrame) -> dict[str, Any]:
    """Calculate comprehensive statistics for PAS benchmark results.
    ...
    pas_totals = _calculate_pas_totals(df)
...
def _format_config_content(config: dict[str, Any]) -> str:
    ...
    # Metrics section (both success/pass@k and PAS metrics)
    content += "\n=== Metrics ===\n"
    k = config["k"]
...
def generate_validation_report(
    ...
    """Generate a validation report for PAS benchmark results.
...
def combine_results_to_dataframe(
    ...
    """Combine multiple PASMultiScenarioValidationResult objects into a single DataFrame.

    Args:
        results: Dictionary mapping (user_model, proactive_model, tool_failure_probability,
                 num_env_events_per_minute) tuples to PASMultiScenarioValidationResult objects.
    ...
    if not dataframes:
        return pl.DataFrame(schema=PAS_RESULT_SCHEMA)
...
def generate_json_stats_report(
    ...
    """Generate a computer-readable JSON report for PAS benchmark results.
