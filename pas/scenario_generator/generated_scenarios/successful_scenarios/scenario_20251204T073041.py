...
Event Planning Professional""",
    ...
    def validate(self, env: AbstractEnvironment) -> ScenarioValidationResult:
        ...
        try:
            ...
            # The agent must use PASAgentUserInterface.send_message_to_user
            # This is an oracle event, so we check if it was executed
            proposal_found = any(
                "send_message_to_user" in str(entry)
                and ("conflict" in str(entry).lower() or "retreat" in str(entry).lower())
                for entry in log_entries
            )

            # Check Step 2: User accepted the proposal
            # The user must use PASAgentUserInterface.accept_proposal
            # This is an oracle event, so we check if it was executed
            acceptance_found = any("accept_proposal" in str(entry) for entry in log_entries)

            # Check Step 3a: Agent replied to the corporate client email
            # The agent must use StatefulEmailApp.reply_to_email
            # This is an oracle event with email_id="corp_retreat_request_001"
            email_reply_found = any(
                "reply_to_email" in str(entry) and "corp_retreat_request_001" in str(entry) for entry in log_entries
            )

            # Check Step 3b: Agent sent message to catering partner
            # The agent must use StatefulMessagingApp.send_message
            # This is an oracle event targeting "+1-555-0202"
            catering_message_found = any(
                "send_message" in str(entry) and "+1-555-0202" in str(entry) for entry in log_entries
            )

            # All oracle events must be present for the scenario to succeed
            success = proposal_found and acceptance_found and email_reply_found and catering_message_found
