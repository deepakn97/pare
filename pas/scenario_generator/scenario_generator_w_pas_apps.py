...
from pas.scenario_generator.agent.seed_scenario_generating_agent_w_pas_apps import SeedScenarioGeneratingAgent
...
def main() -> None:  # noqa: C901
    ...
    parser.add_argument(
        ...
        default="scenario_with_all_pas_apps",
    ...
    parser.add_argument(
        ...
        dest="scale_apps",
        nargs="*",
        default=["StatefulMessagingApp"],
        help=(
            "Explicit list of app class names to use for all generated scenarios "
            "(default: None). "
            "PASAgentUserInterface and HomeScreenSystemApp are always included by default."
        ),
    )

    args = parser.parse_args()

    # Import PAS scenarios so @register_scenario runs before reading registry
    import pas.scenario_generator.example_proactive_scenarios.scenario_with_all_pas_apps
    import pas.scenario_generator.example_proactive_scenarios.very_basic_demo_pas_app  # noqa: F401

    # Also import all user scenarios so their @register_scenario decorators run
    try:
        import importlib
        import pkgutil

        import pas.scenarios.user_scenarios as _user_scenarios_pkg

        for _finder, _modname, _is_pkg in pkgutil.iter_modules(_user_scenarios_pkg.__path__):
