"""PAS-specific scenario exporter with world_logs and proactive context support."""
...
from pas.data_handler.models import (
    PASEventMetadata,
    PASExportedCompletedEvent,
    PASExportedEventMetadata,
    PASExportedTrace,
)
...
class PASJsonScenarioExporter(JsonScenarioExporter):
    ...
    def _get_trace(
        ...
        **kwargs: Any,
        ...
        """Override to use PAS-specific trace conversion.
    ...
    @staticmethod
    def convert_event_metadata(metadata: EventMetadata) -> PASExportedEventMetadata:
        """Convert event metadata, extracting PAS-specific fields if present.
        ...
        if isinstance(metadata, PASEventMetadata):
            proactive_mode = metadata.proactive_mode
            turn_number = metadata.turn_number

        return PASExportedEventMetadata(
    ...
    @staticmethod
    def convert_completed_pas_event(event: CompletedEvent) -> PASExportedCompletedEvent:
        """Override to use PAS-specific metadata conversion.
        ...
        Returns:
            The exported completed event as a dict with PAS metadata fields.
        """
        action = JsonScenarioExporter.convert_action(event.action)

        return PASExportedCompletedEvent(
            ...
            action=action,
            metadata=PASJsonScenarioExporter.convert_event_metadata(event.metadata),
        )
