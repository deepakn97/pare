...
def register_custom_scenarios(registry: ScenarioRegistry) -> None:  # noqa: C901
    ...
    # NOTE: this module lives under `pare/scenarios/generator/`, so we need `parents[2]` to get back to `pare/`.
    ...
    for idx, file_path in enumerate(sorted(discovered_files)):
        try:
            module_name = f"pare.scenarios.generated_dynamic.module_{idx}"
