"""Adapted caching utilities from meta-are/are/simulation/scenarios/utils/caching.py for PARE scenarios.

The changes are minimal and primarily involve updating imports and type hints to reference PARE-specific classes.
"""
...
def get_run_id(scenario: PAREScenario, runner_config: ScenarioRunnerConfig | None) -> str:
...
class CachedScenarioResult:
    ...
    def from_scenario_result(
        cls,
        scenario_result: PAREScenarioValidationResult,
    ...
    def to_scenario_result(self) -> PAREScenarioValidationResult:
        """Convert back to a PAREScenarioValidationResult.

        Returns:
            A PAREScenarioValidationResult instance.
        """
        exception = None
...
def _generate_cache_key(runner_config: ScenarioRunnerConfig, scenario: PAREScenario) -> str:
...
def _generate_scenario_hash(scenario: PAREScenario) -> str:
...
def _get_cache_dir() -> Path:
    ...
    2. Persistent config file setting (~/.config/pare/config.json)
    ...
    cache_dir = os.environ.get("PARE_CACHE_DIR")
    if cache_dir:
        return Path(cache_dir)

    # Check config file
    config_file = Path.home() / ".config" / "pare" / "config.json"
...
def maybe_load_cached_result(
    runner_config: ScenarioRunnerConfig,
    scenario: PAREScenario,
) -> PAREScenarioValidationResult | None:
