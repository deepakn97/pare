...
@register_scenario("project_task_filtering_reminder")
class ProjectTaskFilteringReminder(PAREScenario):
    """Agent synthesizes project tasks from multiple emails, filters user-assigned tasks, and creates deadline reminders.

    The user receives three emails from their project manager Sarah Kim about the upcoming "Mobile App Redesign" project. The first email outlines the overall project timeline and lists five different tasks distributed among the team: UI mockups (assigned to the user, due Nov 25 @ 5:00 PM), backend API updates (assigned to Tom), user testing plan (assigned to the user, due Nov 28 @ 5:00 PM), database migration (assigned to Jennifer), and final presentation deck (assigned to the user, due Dec 1 @ 5:00 PM). The second email clarifies that the user testing plan deadline has moved to Nov 27 @ 5:00 PM. The third email adds that the presentation should be coordinated with Tom. The agent must: 1. Parse all three emails and extract task assignments. 2. Filter tasks specifically assigned to the user (UI mockups, user testing plan, presentation deck). 3. Apply the deadline update from the second email (Nov 27 @ 5:00 PM for user testing). 4. Create three separate reminders for the user's task deadlines. 5. Reply to Sarah's email thread confirming which tasks the user will handle and their deadlines.
...
1. UI mockups - Assigned to you, due Nov 25 @ 5:00 PM
2. Backend API updates - Assigned to Tom, due Nov 26
3. User testing plan - Assigned to you, due Nov 28 @ 5:00 PM
4. Database migration - Assigned to Jennifer, due Nov 29
5. Final presentation deck - Assigned to you, due Dec 1 @ 5:00 PM

Please add reminders for your assigned deadlines.
...
Sarah""",
            ...
            email2_event = email_app.reply_to_email_from_user(
                sender="sarah.kim@company.com",
                email_id="email-project-overview",
                content="""Quick update: The user testing plan deadline has been moved up to Nov 27 @ 5:00 PM (instead of Nov 28 @ 5:00 PM) due to stakeholder availability.

If you've already added a reminder for it, please update it to Nov 27 @ 5:00 PM.
...
It may help to set a reminder for the deck deadline first though on Dec 1 @ 5:00 PM.
...
Sarah""",
            ...
            proposal_event = (
                aui.send_message_to_user(
                    content="I noticed you have three tasks assigned in Sarah's Mobile App Redesign project: UI mockups (due Nov 25 @ 5:00 PM), user testing plan (due Nov 27 @ 5:00 PM), and final presentation deck (due Dec 1 @ 5:00 PM). Would you like me to add reminders for these deadlines?"
                )
                .oracle()
            ...
            # Oracle Event 4: Agent creates reminder for UI mockups deadline (Nov 25 @ 5:00 PM)
            create_reminder1 = (
                reminder_app.add_reminder(
                    title="UI mockups - Mobile App Redesign (due)",
                    due_datetime="2025-11-25 17:00:00",
                    description="Design UI mockups for Mobile App Redesign project (deadline 5:00 PM).",
                )
                .oracle()
                .depends_on(acceptance_event, delay_seconds=1)
            )

            # Oracle Event 5: Agent creates reminder for UPDATED user testing plan deadline (Nov 27 @ 5:00 PM)
            create_reminder2 = (
                reminder_app.add_reminder(
                    title="User testing plan - Mobile App Redesign (due)",
                    due_datetime="2025-11-27 17:00:00",
                    description="Prepare user testing plan for Mobile App Redesign project (deadline 5:00 PM).",
                )
                .oracle()
                .depends_on(create_reminder1, delay_seconds=1)
            )

            # Oracle Event 6: Agent creates reminder for final presentation deck deadline (Dec 1 @ 5:00 PM)
            create_reminder3 = (
                reminder_app.add_reminder(
                    title="Final presentation deck - Mobile App Redesign (due)",
                    due_datetime="2025-12-01 17:00:00",
                    description="Create final presentation deck; coordinate with Tom on API component (deadline 5:00 PM).",
                )
                .oracle()
                .depends_on(create_reminder2, delay_seconds=1)
            )
...
I've reviewed the project assignments and set reminders for my tasks:
- UI mockups: Nov 25 @ 5:00 PM
- User testing plan: Nov 27 @ 5:00 PM (updated deadline noted)
- Final presentation deck: Dec 1 @ 5:00 PM (will coordinate with Tom)
...
Best""",
                ...
                .oracle()
                .depends_on(create_reminder3, delay_seconds=2)
            )
    ...
    def validate(self, env: AbstractEnvironment) -> ScenarioValidationResult:
        ...
        try:
            ...
            # Check Step 2: Agent created reminder for UI mockups deadline (Nov 25 @ 5:00 PM)
            ui_mockup_event_found = any(
                (e.event_type == EventType.AGENT)
                and isinstance(e.action, Action)
                and e.action.class_name == "StatefulReminderApp"
                and e.action.function_name == "add_reminder"
                and "2025-11-25" in e.action.args.get("due_datetime", "")
                and any(keyword in e.action.args.get("title", "").lower() for keyword in ["ui", "mockup"])
                for e in log_entries
            )

            # Check Step 3: Agent created reminder for user testing plan deadline (Nov 27 @ 5:00 PM - UPDATED)
            testing_plan_event_found = any(
                (e.event_type == EventType.AGENT)
                and isinstance(e.action, Action)
                and e.action.class_name == "StatefulReminderApp"
                and e.action.function_name in ["add_reminder", "update_reminder"]
                and "2025-11-27" in e.action.args.get("due_datetime", "")
                and any(
                    keyword in e.action.args.get("title", "").lower() for keyword in ["user", "test", "testing", "plan"]
                )
                for e in log_entries
            )

            # Check Step 4: Agent created reminder for final presentation deck deadline (Dec 1 @ 5:00 PM)
            presentation_event_found = any(
                (e.event_type == EventType.AGENT)
                and isinstance(e.action, Action)
                and e.action.class_name == "StatefulReminderApp"
                and e.action.function_name == "add_reminder"
                and "2025-12-01" in e.action.args.get("due_datetime", "")
                and any(keyword in e.action.args.get("title", "").lower() for keyword in ["presentation", "deck"])
                for e in log_entries
            )

            # Determine success and rationale
            success = proposal_found and ui_mockup_event_found and testing_plan_event_found and presentation_event_found
            ...
            if not success:
                ...
                if not proposal_found:
                    missing_checks.append("agent proposal to add deadline reminders")
                if not ui_mockup_event_found:
                    missing_checks.append("reminder for UI mockups deadline (Nov 25)")
                if not testing_plan_event_found:
                    missing_checks.append("reminder for user testing plan deadline (Nov 27 updated)")
                if not presentation_event_found:
                    missing_checks.append("reminder for final presentation deck deadline (Dec 1)")
