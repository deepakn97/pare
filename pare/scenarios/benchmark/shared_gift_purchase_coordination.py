"""Scenario: Agent coordinates shared gift purchase by aggregating contributions and completing purchase."""
...
class SharedGiftPurchaseCoordination(PAREScenario):
    ...
    additional_system_prompt = """You are coordinating a group wedding gift purchase with Alice and Bob.
Wait for both Alice's and Bob's contribution messages before accepting any proposal.
Only accept a proposal if the agent shows you the available gift options within the pooled budget ($55).
Do not accept a proposal that simply states a gift was found without showing you the options."""
    ...
    def init_and_populate_apps(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.gift_product_id = self.shopping.add_product(name="Ceramic Serving Bowl Set")
        ...
        # Add alternative gift option
        alt_product_id = self.shopping.add_product(name="Wine Glass Set (4-pack)")
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            # Oracle Event 3: Agent checks due reminders to connect the contributions to the wedding-gift deadline.
            ...
            # Oracle Event 4: Agent searches shopping catalog for gift options under the pooled $55 budget.
            ...
            # Oracle Event 5: Agent proposes purchasing a specific gift to the user (requires approval).
            ...
            # Oracle Event 11: Agent sends confirmation to Alice
            # Motivated by: Need to inform Alice that her contribution was used and the gift was purchased
            message_confirmation_event = (
                messaging_app.send_message_to_group_conversation(
                    conversation_id=self.contributions_conv_id,
                    content="The wedding gift for Lisa has been purchased using the pooled contributions from both of you. Thank you!",
                )
                .oracle()
                .depends_on(delete_reminder_event, delay_seconds=2)
            )

            # Oracle Event 13: Agent confirms completion to the user.
            ...
            final_user_event = (
                ...
                .oracle()
                .depends_on([message_confirmation_event], delay_seconds=1)
            )
    ...
    def validate(self, env: AbstractEnvironment) -> ScenarioValidationResult:
        ...
        - Agent completed checkout (purchase was made)
        - Agent sent confirmation messages to both Alice and Bob

        Not checked (intermediate steps the agent might do differently):
        - How agent found products (search vs list)
        - Whether agent checked reminders
        - Whether agent deleted the reminder after purchase
        """
        try:
            ...
            # CHECK 2: Agent completed checkout (essential - the purchase must happen)
            checkout_found = any(
                e.action.class_name == "StatefulShoppingApp" and e.action.function_name == "checkout"
                for e in agent_events
            )

            # CHECK 3: Agent sent confirmation messages to both contributors
            ...
            all_checks = [
                ...
                ("confirmations_sent", group_message_events),
            ]

            failed_checks = [name for name, passed in all_checks if not passed]

            if failed_checks:
                return ScenarioValidationResult(success=False, rationale=f"Failed checks: {', '.join(failed_checks)}")
