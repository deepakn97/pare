...
class RestockAlertAutoPurchase(PAREScenario):
    ...
    def init_and_populate_apps(self, *args: Any, **kwargs: Any) -> None:
        ...
        # Add the item to the user's cart (simulating previous failed attempt) and then mark it as out of stock
        self.shopping.add_to_cart(item_id=self.item_id, quantity=1)
        self.shopping.update_item(item_id=self.item_id, new_availability=False)

        # Update cart item availability to reflect out-of-stock status
        if self.item_id in self.shopping.cart:
            self.shopping.cart[self.item_id].available = False
