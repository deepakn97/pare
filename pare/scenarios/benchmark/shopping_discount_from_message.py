"""Scenario: Agent applies discount code from message to shopping cart."""
...
class ShoppingDiscountFromMessage(PAREScenario):
    ...
    def init_and_populate_apps(self, *args: Any, **kwargs: Any) -> None:
        ...
        if self.friend_id is None:
            raise RuntimeError("Failed to get friend_id for Alex Chen")
        ...
        # Create products using app methods
        # Product 1: Wireless Mouse
        mouse_product_id = self.shopping.add_product(name="Wireless Mouse")
        ...
        cable_product_id = self.shopping.add_product(name="USB-C Cable")
        ...
        self.shopping.add_to_cart(item_id=self.mouse_item_id, quantity=1)
        self.shopping.add_to_cart(item_id=self.cable_item_id, quantity=1)

        # Add discount code that applies to both items
        self.shopping.add_discount_code(item_id=self.mouse_item_id, discount_code={"TECH20": 20.0})
        self.shopping.add_discount_code(item_id=self.cable_item_id, discount_code={"TECH20": 20.0})
    ...
    def build_events_flow(self) -> None:
        ...
        # Get conversation ID for the friend
    ...
    def validate(self, env: AbstractEnvironment) -> ScenarioValidationResult:
        ...
        - Agent completed checkout with TECH20 discount code
        - Agent sent thank-you message to friend

        Not checked (intermediate steps the agent might do differently):
        - How agent read the conversation
        - Whether agent listed cart contents
        - Whether agent verified discount code applicability
        """
        try:
            log_entries = env.event_log.list_view()

            # CHECK 1: Agent sent proposal to user about applying discount code
            proposal_found = any(
            ...
            # CHECK 3: Agent sent thank-you message to friend
            thank_you_found = any(
                e.event_type == EventType.AGENT
                and isinstance(e.action, Action)
            ...
            success = proposal_found and checkout_found and thank_you_found

            if not success:
                failed_checks = []
                if not proposal_found:
                    failed_checks.append("agent did not send proposal to user about discount code")
                if not checkout_found:
                    failed_checks.append("agent did not complete checkout with TECH20 discount code")
                if not thank_you_found:
                    failed_checks.append("agent did not send thank-you message to friend")
