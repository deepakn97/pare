"""Scenario: Agent detects apartment price drop and alerts user based on calendar timeline."""
...
class ApartmentPriceDropTimelineAlert(PAREScenario):
    ...
    def init_and_populate_apps(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.calendar.add_calendar_event(
            ...
            end_datetime="2025-12-15 12:00:00",
    ...
    def validate(self, env: AbstractEnvironment) -> ScenarioValidationResult:
        ...
        - Agent sent proposal to user about the price drop opportunity
        - Agent sent email to listing agent after user acceptance

        Not checked (intermediate steps the agent might do differently):
        - How agent discovered saved apartments (list_saved_apartments, search, etc.)
        ...
        try:
            ...
            # CHECK 2: Agent sent email to listing agent
            # Accept: send_email to sarah.martinez@sunsetplaza.com OR reply_to_email to price-drop-email-001
            email_to_listing_agent_found = False
            for e in log_entries:
                if e.event_type != EventType.AGENT or e.action.class_name != "StatefulEmailApp":
                    continue

                args = e.action.args if e.action.args else e.action.resolved_args

                if e.action.function_name == "send_email" and "sarah.martinez@sunsetplaza.com" in str(
                    args.get("recipients", [])
                ):
            ...
            success = proposal_found and email_to_listing_agent_found

            if not success:
                failed_checks = []
                if not proposal_found:
                    failed_checks.append("agent did not send proposal to user about price drop")
                if not email_to_listing_agent_found:
                    failed_checks.append("agent did not contact listing agent")
