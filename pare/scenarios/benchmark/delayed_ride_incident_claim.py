...
class DelayedRideIncidentClaim(PAREScenario):
    """Agent mitigates a delayed cab ride by canceling and booking a replacement ride.
    ...
    2. Propose canceling the delayed ride and booking a replacement ride
    3. After user acceptance, cancel the delayed ride and book the replacement ride for the same route
    4. Retrieve the new ride status to confirm the replacement booking

    This scenario exercises delay-triggered decision making, cab cancellation + rebooking, and ride-status verification for a time-sensitive appointment.
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            # Motivation: delay email requires immediate action and explicitly recommends canceling and rebooking.
            proposal_event = (
                aui.send_message_to_user(
                    content="I received an urgent email from CabApp Support: your ride R2847 (789 Pine Street → Medical Center, 456 Oak Avenue) is delayed by 45+ minutes and may cause you to miss your 4:00 PM appointment. They recommend canceling and booking a replacement ride immediately for the same route. Should I cancel the delayed ride and book the replacement ride now?"
                )
                .oracle()
            ...
            # Motivation: confirm the replacement ride is booked and active.
