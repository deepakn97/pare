...
class RoommateApartmentSearchFromChat(PAREScenario):
    ...
    def init_and_populate_apps(self, *args: Any, **kwargs: Any) -> None:
        ...
        # Save apartment IDs for use in event flow
        self.matching_apartment_ids = []

        apt_id_1 = self.apartment.add_new_apartment(
            name="Downtown Lofts",
            location="Downtown",
            zip_code="90012",
            price=2400.0,
        ...
        self.matching_apartment_ids.append(apt_id_1)

        apt_id_2 = self.apartment.add_new_apartment(
            name="Metro Heights",
            location="Downtown",
            zip_code="90013",
            price=2300.0,
        ...
        self.matching_apartment_ids.append(apt_id_2)

        apt_id_3 = self.apartment.add_new_apartment(
        ...
        self.matching_apartment_ids.append(apt_id_3)
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            # Oracle events: Agent saves the top 3 matching apartments to favorites
            # This matches the scenario description requirement to "Save the top 3-4 matching apartments to favorites"
            save_apt1 = (
                apartment_app.save_apartment(apartment_id=self.matching_apartment_ids[0])
                .oracle()
                .depends_on(oracle2, delay_seconds=1)
            )
            save_apt2 = (
                apartment_app.save_apartment(apartment_id=self.matching_apartment_ids[1])
                .oracle()
                .depends_on(save_apt1, delay_seconds=0.5)
            )
            save_apt3 = (
                apartment_app.save_apartment(apartment_id=self.matching_apartment_ids[2])
                .oracle()
                .depends_on(save_apt2, delay_seconds=0.5)
            )
            ...
            # After saving apartments to favorites, agent proposes sharing details with the group
            proposal = (
                ...
                .oracle()
                .depends_on([save_apt3, oracle2, env1, env2], delay_seconds=1)
            )
    ...
    def validate(self, env: AbstractEnvironment) -> ScenarioValidationResult:
        ...
        try:
            ...
            if not proposal_found:
                rationale = "Agent did not send a proposal to the user via PAREAgentUserInterface.send_message_to_user"
                success = False
