"""Scenario: Agent creates calendar event for product delivery based on order confirmation."""
...
class ShoppingOrderCalendarSyncDelivery(PAREScenario):
    ...
    def validate(self, env: AbstractEnvironment) -> ScenarioValidationResult:
        ...
        - Agent created calendar event with correct delivery time window (Dec 22, 2025, 2-4 PM)

        Not checked (intermediate steps the agent might do differently):
        - How agent read the email (list_emails vs get_email_by_id)
        - Whether agent checked calendar for conflicts
        """
        try:
            log_entries = env.event_log.list_view()
            agent_events = [e for e in log_entries if e.event_type == EventType.AGENT]

            # CHECK 1: Agent sent proposal to user about creating calendar event
            proposal_found = any(
            ...
            # CHECK 2: Agent created calendar event with correct delivery time window
            ...
            success = proposal_found and create_calendar_event_found

            if not success:
                failed_checks = []
                if not proposal_found:
                    failed_checks.append("agent did not send proposal to user about delivery calendar event")
                if not create_calendar_event_found:
                    failed_checks.append(
                        "agent did not create calendar event with correct delivery time (Dec 22, 2025, 2-4 PM)"
                    )
