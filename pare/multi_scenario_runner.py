"""Multi-scenario runner for PARE benchmark execution."""
...
from pare.constants import RETRYABLE_ERRNOS
from pare.scenario_runner import TwoAgentScenarioRunner
from pare.scenarios.config import MultiScenarioRunnerConfig, ScenarioRunnerConfig
from pare.scenarios.validation_result import PAREMultiScenarioValidationResult, PAREScenarioValidationResult
...
def _is_retryable_error(
    ...
    result: PAREScenarioValidationResult | None,
) -> bool:
...
def maybe_run_scenario(
    ...
    if enable_caching:
        from pare.scenarios.utils.caching import maybe_load_cached_result
    ...
    if enable_caching:
        from pare.scenarios.utils.caching import write_cached_result
...
class MultiScenarioRunner:
    """Runner for executing multiple PARE scenarios with shared configuration."""
    ...
    def run(
        self, config: MultiScenarioRunnerConfig, scenarios: list[PAREScenario]
    ...
    def _run_phase(
        self,
        scenarios: CountableIterator[PAREScenario] | list[PAREScenario],
        config: MultiScenarioRunnerConfig,
        max_workers: int,
        multi_result: PAREMultiScenarioValidationResult,
    ...
    def run_with_scenarios(  # noqa: C901
        self,
        config: MultiScenarioRunnerConfig,
        scenarios: CountableIterator[PAREScenario],
        ...
        Returns:
            The combined validation result for all scenarios.
        """
        multi_result = PAREMultiScenarioValidationResult(run_config=config)
