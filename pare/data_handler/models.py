"""PARE-specific models for trajectory data."""
...
@dataclass
class PAREEventMetadata(EventMetadata):
...
class PAREExportedCompletedEvent(BaseModel):
    """PARE-specific exported completed event with PAREExportedEventMetadata.
    ...
    PAREExportedEventMetadata, we ensure proactive_mode and turn_number are serialized.
    """
    ...
    metadata: PAREExportedEventMetadata | None = None


class PAREExportedTrace(ExportedTraceBase):
    """PARE-specific exported trace with PAREExportedCompletedEvent.

    This ensures proactive_mode and turn_number are serialized correctly.
    """

    metadata: ExportedTraceMetadata
    completed_events: list[PAREExportedCompletedEvent]
