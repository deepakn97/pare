...
1. **Export world_logs**: Create `PAREJsonScenarioExporter` that properly exports agent logs
2. **Add proactive_mode metadata**: Enrich CompletedEvent metadata with proactive agent state

---

## Change 1: PARE-Specific Exporter with world_logs
...
### Solution
Create `PAREJsonScenarioExporter` in `pare/data_handler/exporter.py`
...
pare/
  data_handler/
    __init__.py
    exporter.py
    models.py  # PARE-specific export models
```

#### Step 2: Implement PAREJsonScenarioExporter

```python
# pare/data_handler/exporter.py
...
#### Step 1: Create PARE export models

```python
# pare/data_handler/models.py

from pydantic import BaseModel
from are.simulation.data_handler.models import ExportedEventMetadata

class PAREExportedEventMetadata(ExportedEventMetadata):
...
```python
# pare/agents/proactive/agent.py
...
class PAREJsonScenarioExporter(JsonScenarioExporter):
    ...
    def convert_completed_event(
        ...
        if proactive_context and event.event_id in proactive_context:
            ...
            base_exported.metadata = PAREExportedEventMetadata(
...
| File | Change |
|------|--------|
| `pare/data_handler/__init__.py` | New - exports |
| `pare/data_handler/exporter.py` | New - `PAREJsonScenarioExporter` |
| `pare/data_handler/models.py` | New - `PAREExportedEventMetadata`, `ProactiveEventContext` |
| `pare/environment.py` | Add proactive context tracking |
| `pare/scenario_runner.py` | Use new exporter, set proactive context before agent turns |

---

## Implementation Order

1. Create `pare/data_handler/` module with models
2. Implement `PAREJsonScenarioExporter` with world_logs fix
