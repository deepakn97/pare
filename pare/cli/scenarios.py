"""Scenario CLI commands for listing and generating PARE scenarios."""
...
app = typer.Typer(
    name="scenarios",
    help="List registered PARE scenarios and run the multi-step scenario generator",
    no_args_is_help=True,
)


_SCENARIOS_BASE_DIR = Path(__file__).resolve().parents[1] / "scenarios"
_SCENARIO_METADATA_PATH = _SCENARIOS_BASE_DIR / "scenario_metadata.json"
...
def _configured_scenarios_dir_inputs(scenarios_dirs: list[str] | None = None) -> list[str]:
    ...
    if not configured:
        env_value = os.getenv("PARE_SCENARIOS_DIR", "benchmark")
...
class ScenarioListing:
    """Lightweight representation of a scenario for CLI output."""
...
def _reset_pare_registry_discovery_state() -> None:
    from pare.scenarios import registry
...
def _temporary_scenarios_dirs(scenarios_dirs: list[str] | None = None) -> Iterator[list[Path]]:
    ...
    previous_dirs = os.environ.get("PARE_SCENARIOS_DIR")

    try:
        os.environ["PARE_SCENARIOS_DIR"] = ",".join(scenarios_dir_names)
        _reset_pare_registry_discovery_state()
        yield resolved_dirs
    finally:
        if previous_dirs is None:
            os.environ.pop("PARE_SCENARIOS_DIR", None)
        else:
            os.environ["PARE_SCENARIOS_DIR"] = previous_dirs
        _reset_pare_registry_discovery_state()
...
def _load_apps_for_listing(scenario_id: str, scenario: PAREScenario, metadata: dict[str, dict[str, Any]]) -> list[str]:
...
def _load_registered_scenario_listings(scenarios_dirs: list[str] | None = None) -> list[ScenarioListing]:
    from pare.scenarios import registry
...
def scenarios_root(
    ...
    list_only: Annotated[
        bool,
        typer.Option("--list", help="Alias for `pare scenarios list`"),
    ] = False,
    scenarios_dirs: Annotated[
        ...
        typer.Option(
            ...
            "--benchmark-dir",
            help=(
                "Scenario directories under pare/scenarios/ (repeat or comma-separate). "
                "Defaults to PARE_SCENARIOS_DIR or benchmark."
            ),
        ),
    ] = None,
    apps: Annotated[
    ...
    """Handle root-level shortcuts such as `pare scenarios --list`."""
...
def _scenario_to_listing(scenario: PAREScenario, metadata: dict[str, dict[str, Any]]) -> ScenarioListing:
...
def _emit_listings(listings: list[ScenarioListing], *, as_json: bool) -> None:
    if as_json:
        payload = [
            {
...
def list_scenarios(
    ...
    """List registered PARE scenarios from configured scenario directories."""
...
def check_ids_file(
    ...
    """Validate that scenario IDs from a file exist in the configured PARE scenario directories."""
...
def generate(  # noqa: C901
    ...
    model: Annotated[
        str,
        typer.Option("--model", help="(Currently unused) Kept for parity with the generator entrypoint"),
    ] = "gpt-5-mini-2025-08-07",
    provider: Annotated[
        str,
        typer.Option("--provider", help="(Currently unused) Kept for parity with the generator entrypoint"),
    ] = "openai",
    endpoint: Annotated[
        str | None,
    ...
    apps: Annotated[
        ...
        typer.Option(
            ...
            help=(
                ...
                "PAREAgentUserInterface and HomeScreenSystemApp are always available."
            ),
        ),
    ] = None,
    as_json: Annotated[
    ...
    from pare.scenarios.generator.scenario_generator import determine_selected_apps, prepare_prompt_context_data
    ...
    requested_apps = _expand_csv_list(apps) if apps is not None else None

    app_def_scenario = ScenarioWithAllPAREApps()
    app_def_scenario.initialize()
    app_instances = {
        app_instance.__class__.__name__: app_instance for app_instance in getattr(app_def_scenario, "apps", [])
    }
            ...
            if isinstance(step_results, list):
                ...
                for step in step_results:
                    if isinstance(step, dict):
                        ...
                        compact_step.pop("conversation", None)
