...
class CancelledMeetingNoteCleanup(PASScenario):
    """Agent cleans up preparation notes when calendar events are cancelled by organizers. The user has a note in their "Work" folder titled "Client Onboarding Meeting Prep - Nov 28" containing detailed preparation materials, agenda items, and attendee information for an upcoming client meeting scheduled for November 28th at 3:00 PM. The meeting organizer sends a calendar cancellation notification due to the client's scheduling conflict. The agent must: 1. Recognize the calendar event cancellation notification 2. Search notes for content matching the cancelled meeting title and date 3. Identify the now-obsolete preparation note 4. Propose moving the note to a different folder or deleting it to avoid clutter 5. Update or archive the note based on user preference 6. Confirm with the user that the workspace has been cleaned up.
...
- Set up regular check-in schedule""",
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            # The agent observes the calendar cancellation notification and searches for related notes
            # Agent searches notes for content matching the cancelled meeting
            search_notes_event = (
                note_app.search_notes(query="Client Onboarding Meeting Nov 28")
                .oracle()
                .depends_on(cancellation_event, delay_seconds=3)
            )

            # Agent proposes cleanup action based on the cancellation notification
            proposal_event = (
                aui.send_message_to_user(
                    content="I noticed that the Client Onboarding Meeting scheduled for November 28th has been cancelled by Sarah Chen. You have a preparation note for this meeting in your Work folder. Would you like me to delete this note since the meeting is no longer happening?"
                )
                .oracle()
