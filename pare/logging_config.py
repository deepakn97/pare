...
class PAREFormatter(ScenarioAwareFormatter):
...
def configure_logging(
    ...
    """
    standard_formatter = PAREFormatter()
    ...
    if log_dir is not None:
        if scenario_id is None:
            ...
            for logger_name in ["are.simulation.agents", "pare.agents"]:
...
class _ExcludeAgentLogsFilter(logging.Filter):
    """Filter to exclude agent logs from being logged to the PARE log file."""

    def filter(self, record: logging.LogRecord) -> bool:
        return not (record.name.startswith("are.simulation.agents") or record.name.startswith("pare.agents"))
