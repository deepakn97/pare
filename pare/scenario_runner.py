...
from pare.agents.agent_builder import ProactiveAgentBuilder, UserAgentBuilder
from pare.agents.agent_config_builder import ProactiveAgentConfigBuilder, UserAgentConfigBuilder
from pare.apps import StatefulApp
from pare.data_handler.exporter import PAREJsonScenarioExporter
from pare.environment import StateAwareEnvironmentWrapper
from pare.notification_system import PARENotificationSystem
...
if TYPE_CHECKING:
    ...
    from pare.agents.pare_agent_config import ProactiveObserveExecuteAgentConfig, UserDefaultAgentConfig
...
def _count_observe_mode_acceptances(completed_events: list[CompletedEvent]) -> int:
    ...
    for event in completed_events:
        ...
        if app_class != "PAREAgentUserInterface":
            continue
...
class TwoAgentScenarioRunner:
    ...
    PARE scenarios. It supports both normal mode (with agents) and oracle mode
    ...
    def _run_without_agent(
        ...
        # Convert to PAREScenarioValidationResult (no agent metrics in oracle mode)
        return PAREScenarioValidationResult(
    ...
    def _export_pare_trace(
        ...
        execute_model: str,
        proactive_agent: str,
        validation_result: PAREScenarioValidationResult,
        run_duration: float,
        ...
        """Export the trace of the PARE scenario.
        ...
        scenario_exporter = PAREJsonScenarioExporter()
    ...
    def _run_with_two_agents(
    ...
    ) -> tuple[PAREScenarioValidationResult, UserAgent | None, ProactiveAgent | None]:
        ...
        # Convert to PAREScenarioValidationResult with metrics
        validation_result = PAREScenarioValidationResult(
            success=base_result.success,
            exception=base_result.exception,
            export_path=base_result.export_path,
    ...
    def _run_pare_scenario(
        self,
        config: ScenarioRunnerConfig,
        scenario: PAREScenario,
    ) -> PAREScenarioValidationResult:
        """Run a Proactive Agents Research Environment scenario.
        ...
        Returns:
            PAREScenarioValidationResult: The validation result of the scenario.
        """
        ...
        env = StateAwareEnvironmentWrapper(
            ...
            config=env_config,
            notification_system=PARENotificationSystem(verbosity_level=VerbosityLevel.HIGH),
        )
        ...
        try:
            ...
            validation_result, user_agent, proactive_agent = (
                PAREScenarioValidationResult(success=None, exception=exception),
                None,
                None,
            )
        ...
        # CLAUDE CODE: we need to implement the _export_pare_trace method to export everything.
        try:
            has_hf_metadata = getattr(scenario, "hf_metadata", None) is not None
            export_path = None
            if user_agent is not None and proactive_agent is not None:
                export_path = self._export_pare_trace(
                    env,
                    scenario,
                    user_agent.model,
    ...
    def run(
        ...
        try:
            result = self._run_pare_scenario(config, scenario)
        except Exception as exception:
            logger.exception("Failed to run scenario")
            result = PAREScenarioValidationResult(success=None, exception=exception)
