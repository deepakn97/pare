...
PARE_USER_ENVIRONMENT_INSTRUCTIONS = textwrap.dedent(
  ...
  - PAREAgentUserInterface is a special app that allows you to communicate with the proactive agent. The tools from this app are always available. You CANNOT switch to this app, but you can use the tools from this app from anywhere.

  SYSTEM RESPONSE TOOLS (always available):
  - accept_proposal() and reject_proposal() are always available.
  - These are always-accessible tools. You don't need to navigate to the PAREAgentUserInterface app to use these tools.
...
DEFAULT_USER_AGENT_SYSTEM_PROMPT = USER_AGENT_SYSTEM_PROMPT_TEMPLATE.format(
    ...
    environment_instructions=PARE_USER_ENVIRONMENT_INSTRUCTIONS,
)
