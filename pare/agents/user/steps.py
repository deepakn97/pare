...
from pare.agents.agent_log import USER_AGENT_DYNAMIC_LOG_TYPES, AgentMessageLog, AvailableToolsLog, CurrentAppStateLog
...
def pull_notifications_and_tools(agent: BaseAgent) -> None:
    ...
    for notification in unhandled_notifications:
        if notification.message_type == PAREMessageType.AGENT_MESSAGE:
            agent_messages.append(notification)
        elif notification.message_type == PAREMessageType.ENVIRONMENT_NOTIFICATION_USER:
