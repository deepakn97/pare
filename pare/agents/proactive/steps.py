...
from pare.agents.agent_log import UserActionLog
...
def pull_proactive_agent_messages(agent: BaseAgent) -> None:
    ...
    for notification in unhandled_notifications:
        if notification.message_type == PAREMessageType.ENVIRONMENT_NOTIFICATION_AGENT:
            env_notifications.append(notification)
        elif notification.message_type == PAREMessageType.USER_ACTION:
